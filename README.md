...
本项目交付形态是 Android APK，内部通过 WebView 加载本地 H5 页面，由 H5 Canvas 完成图像处理。用户从手机本地选择图片后，页面展示原图预览，并按 `gray = 0.299R + 0.587G + 0.114B` 逐像素计算灰度值，统计 `0-255` 共 256 个 bins，再按最大计数归一化到 `0-100`，绘制 `256x100` 黑白直方图。界面会显示本次直方图生成耗时，并提供 bins、像素总数、归一化和离线能力等验收自检信息。系统离线运行，无后端、无数据库、无网络依赖，适合课程现场安装、选图、生成结果和解释实现过程。
...
| 性能目标 | 显示生成耗时，直方图生成耗时应控制在 300ms 以内 |
| 离线能力 | APK 内置 assets，无后端、无数据库、无网络依赖 |
| 产品观感 | 黑紫控制台视觉、统一图标、自然 loading、答辩友好分屏 |
| 对照交付 | 低效对照版本 / 正式演示版本两份可并装 debug APK，用于性能答辩对比 |
...
- **性能可讲清**：保留低效对照版本与当前优化版本的同边界 benchmark，答辩时能解释为什么优化有效。
- **离线可演示**：核心 assets 打包进 APK，本地选择图片即可完成预览、统计、绘制和耗时展示。
- **答辩友好 UI**：正式演示版本界面按接入、链路、输出、指标、协议、日志拆分，便于讲解处理流程和证据。
...
| 图标 | 顶部 logo、底部 tab、上传按钮、架构节点统一为线性工程风格；两个 APK 用右下角角标区分低效对照版本/正式演示版本 |
| 动画 | 开屏、页面切换、loading、按钮反馈统一缓动节奏，降低突兀闪动 |
| 答辩展示 | 接入、链路、输出、指标、协议、日志六页拆分，便于逐页截图进 PPT |

## 演示路径

1. 安装正式演示版本 APK：`dist/mobile-histogram-optimized-coinstall-debug.apk`。
2. 关闭网络或保持离线环境，打开应用。
3. 点击“接入图像”，选择手机本地图片。
4. 查看原图预览，并切换到输出页查看 `256x100` 黑白直方图。
5. 在指标页说明 bins 总数、像素总数、最大计数、是否低于 `300ms` 和一致性校验。
6. 如需对比，安装 `dist/mobile-histogram-baseline-coinstall-debug.apk`，用同一张图片比较低效对照版本与当前优化版本耗时。
...
| 性能对照 | 低效对照版本与当前优化版本同边界输出一致性和耗时对比 | `docs/测试人员/stage5-performance-comparison.md` |
...
直方图生成耗时应控制在 300ms 以内。当前仓库同时保留 Node.js 算法 benchmark 和 Android 真机端到端测试模板：前者只用于验证算法策略差异，后者用于最终验收报告回填。

### Node.js 算法 benchmark

以下表格来自开发机 Node.js 合成 RGBA 数据。它用于说明普通数组、对象分配、重复扫描等实现策略会增加耗时，不替代 Android 真机端到端测试。
...
- 详细记录见 [stage5-performance-comparison.md](docs/测试人员/stage5-performance-comparison.md)。

### Android 真机端到端测试

真机测试需要在正式演示版本 APK 中完成“打开应用 -> 选择本地图片 -> 原图预览 -> 生成直方图 -> 查看耗时”的完整路径。当前仓库尚未记录完整真机数据，先保留待回填模板；截图或录屏建议放入 `docs/assets/` 后在表格备注中链接。

| 设备 | Android版本 | 图片尺寸 | 像素数 | 生成耗时 | 是否低于300ms | 备注 |
| --- | --- | --- | ---: | --- | --- | --- |
...
| 产物 | 路径 | 用途 |
| --- | --- | --- |
| 正式演示版本 APK | `dist/mobile-histogram-optimized-coinstall-debug.apk` | 当前优化版本，用于主演示 |
| 低效压力对照版本 APK | `dist/mobile-histogram-baseline-coinstall-debug.apk` | 性能对照版本，用于说明低效策略影响 |
| 第 4 阶段 APK | `dist/mobile-histogram-stage4-debug.apk` | 基础主流程交付记录 |
| 性能证据 | `docs/测试人员/stage5-performance-comparison.md` | benchmark 与 APK 哈希 |
| 离线自测 | `docs/测试人员/stage3-algorithm-offline-self-test.md` | 算法与离线边界证据 |
| APK 交付 | `docs/测试人员/stage4-apk-handoff.md` | 打包命令、包信息、待回填项 |
...
- 低效对照版本：`http://127.0.0.1:4173/index-baseline.html`
...
- Android assets 离线边界；
- 低效对照版本与当前优化版本输出一致性。
- `app/src` 下 JS、Java、CSS、HTML、XML 源码/资源代码均有中文分段注释，当前优化版本核心源码注释/代码比例超过三分之一。
...
├── app/
│   └── src/main/
│       ├── assets/            # H5 页面、样式、正式演示版本与低效对照脚本
│       ├── java/              # Android WebView 壳与图片保存桥
│       └── res/               # 图标、启动背景、样式资源
├── dist/                      # 已输出的 debug APK
├── docs/
│   ├── assets/                # README 与答辩截图素材
│   ├── 测试人员/              # 阶段测试、离线验证、性能对照证据
│   ├── 研发人员/              # 技术设计、PRD 评审、任务清单
...
- [研发技术设计](docs/研发人员/tech-design.md)
- [PRD 评审记录](docs/研发人员/prd-review.md)
- [研发任务清单 tasks-list.md](docs/研发/tasks-list.md)
- [算法与离线自测证据](docs/测试人员/stage3-algorithm-offline-self-test.md)
- [APK 打包交付记录](docs/测试人员/stage4-apk-handoff.md)
- [性能对照实验记录](docs/测试人员/stage5-performance-comparison.md)
- [答辩常见问题](docs/答辩/常见问题.md)
...
- [x] 输出基础 APK、低效压力对照版本 APK 和正式演示版本 APK，证据见 `dist/` 与 [stage5-performance-comparison.md](docs/测试人员/stage5-performance-comparison.md)。
- [x] 建立低效对照版本与当前优化版本的同边界性能对照证据，证据见 [stage5-performance-comparison.md](docs/测试人员/stage5-performance-comparison.md)。
- [x] 补齐测试计划、测试用例、执行记录和测试报告初稿，证据见 `docs/测试人员/`。
- [x] 补齐使用说明，证据见 [使用说明.md](docs/需求分析师/使用说明.md)。
- [x] 补充答辩常见问题，证据见 [常见问题.md](docs/答辩/常见问题.md)。

### 最终提交前待补充

- [ ] 补齐真机安装验证记录、设备信息、图片尺寸、耗时和截图。
- [ ] 回填设备型号、Android 版本、测试图片尺寸、实测耗时和是否低于 `300ms`。
- [ ] 补充真机截图或录屏到 `docs/assets/`。
- [ ] 将真机数据汇总进最终测试报告。
- [ ] 整理答辩 PPT 和演示脚本。
...
5. `性能对比`：说明低效对照版本与当前优化版本的同边界 benchmark。
